...
var petTypeController = function () {
    ...
    var apiService = function (apiURI) {
        var getById = function (Id) {
            return $.ajax({
                url: apiURI + "PetClassification/" + Id + "/detail",
                data: { Id: Id },
                type: "GET",
    ...
    var dataTable, dataTableSelectPetType, dataTableSelectGroup;
    var appSettings = {
        model: {},
        status: { IsNew: false },
        currentId: null
    ...
    var init = function (obj) {
        ...
        var petTypeTemplate = $.templates('#petType-template');
        petTypeTemplate.link("#selected-type", { PetTypeId: null, PetType: {} });
        initEvent();
    ...
    var iniValidation = function () {
        form.validate({
            ...
            messages: {
                ...
                PetClassificationId: "Please select Pet Classification Id"
            },
            errorElement: 'span',
    ...
    var initEvent = function () {
        ...
        $("#table-petClassification tbody").on("click", "tr .dropdown-menu a.edit", Edit);
        $("#table-petClassification tbody").on("click", "tr .dropdown-menu a.remove", Delete);
    };
    ...
    var initGrid = function () {
        ...
        dataTable = $("#table-petClassification").DataTable({
            processing: true,
            responsive: true,
            columnDefs: [
                {
            ...
            "columns": [
                { "data": "PetClassificationId", "sortable": false, "orderable": false, "searchable": false },
                {
                ...
                { "data": "PetType.Name" },
                {
                    ...
                    render: function (data, type, full, meta) {
                        return '<span class="dropdown pmd-dropdown dropup clearfix">'
                            + '<button class="btn btn-sm pmd-btn-fab pmd-btn-flat pmd-ripple-effect btn-danger" type="button" id="drop-role-' + full.PetClassificationId + '" data-toggle="dropdown" aria-expanded="true">'
                            + '<i class="material-icons pmd-sm">more_vert</i>'
                            + '</button>'
                            + '<ul aria-labelledby="drop-role-' + full.PetClassificationId + '" role="menu" class="dropdown-menu pmd-dropdown-menu-top-right">'
                            + '<li role="presentation"><a class="edit" style="color:#000" href="javascript:void(0);" tabindex="-1" data-value="' + full.PetClassificationId + '" role="menuitem">Edit</a></li>'
                            + '<li role="presentation"><a class="remove" style="color:#000" href="javascript:void(0);" tabindex="-1" data-value="' + full.PetClassificationId + '" role="menuitem">Remove</a></li>'
                            + '</ul>'
                            + '</span>'
                    }
                }
            ...
            "serverSide": true,
            "ajax": {
                "url": app.appSettings.petDatingWebURI + "/PetClassification/GetPage",
                "type": "GET",
                "datatype": "json",
                data: function (data) {
                    var dataFilter = {
                        draw: data.draw,
                        PetTypeId: appSettings.SelectedPetType.PetTypeId,
    ...
    var ShowSelectGroup = function () {
        ...
        $("#modal-dialog-selectPetGroup").find('.modal-footer #btnSelect').on("click", function () {
            if (appSettings.SelectedPetGroup !== null) {
                ...
                app.appSettings.HasPetGroupSelected = true;
                ShowSelectPetType();
            }
        });
        //show modal
    ...
    var ShowSelectPetType = function () {
        if (!app.appSettings.HasPetGroupSelected) {
            Swal.fire("Not Allowed!", "Please select Pet Group", 'error');
            return false;
        }
        $("#modal-dialog-selectPetType").find('.modal-title').html('Select Pet Type');
        $("#modal-dialog-selectPetType").find('.modal-footer #btnSelect').html('SELECT');
        $("#modal-dialog-selectPetType").find('.modal-footer #btnSelect').attr("data-name", "SELECT");
        $("#modal-dialog-selectPetType").find('.modal-footer #btnSelect').prop("disabled", true);

        initGridPetType();
        var petTypeTemplate = $.templates('#petType-template');
        $("#modal-dialog-selectPetType").find('.modal-footer #btnSelect').on("click", function () {
            if (appSettings.SelectedPetType !== null) {
                circleProgress.show();
                petTypeTemplate.link("#selected-type", appSettings.SelectedPetType);
                $("#btnChangeType").on("click", ShowSelectPetType);
                $("#modal-dialog-selectPetType").modal('hide');
                app.appSettings.HasPetTypeSelected = true;
                initGrid();
            }
        });
        $("#modal-dialog-selectPetType").find('.modal-close').on("click", function () {
            circleProgress.show();
            ShowSelectGroup();
            $("#modal-dialog-selectPetType").modal('hide');
        });
        //show modal
        $("#modal-dialog-selectPetType").modal('show');
        //end show modal
    }

    var initGridPetType = function () {
        if (dataTableSelectPetType) {
            dataTableSelectPetType.destroy();
        }
        circleProgress.show();
        dataTableSelectPetType = $("#table-petType").DataTable({
            processing: true,
            responsive: true,
            columnDefs: [
                {
                    targets: [0, 3, 4], className: "hidden",
                },
                {
                    targets: [1], width: 1
                }
        ...
        dataTableSelectPetType.on('select', function (e, dt, type, index) {
            if (type == 'row') {
                appSettings.SelectedPetType = dataTableSelectPetType.rows(index).data()[0];
                $("#modal-dialog-selectPetType").find('.modal-footer #btnSelect').prop("disabled", false);
            }
        });
        dataTableSelectPetType.on('deselect', function (e, dt, type, index) {
            if (type == 'row') {
                //$.observable(appSettings.SelectedPetType).setProperty(null);
                appSettings.SelectedPetType = null;
                $("#modal-dialog-selectPetType").find('.modal-footer #btnSelect').prop("disabled", true);
            }
        })
        dataTableSelectPetType.columns.adjust();

    }

    var Add = function () {
        if (!app.appSettings.HasPetTypeSelected) {
            Swal.fire("Not Allowed!", "Please select Pet Type", 'error');
            return false;
        }
        appSettings.status.IsNew = true;
        var petClassificationTemplate = $.templates('#petClassification-template');
        $("#modal-dialog").find('.modal-title').html('New Pet Classification');
        ...
        appSettings.model = {
            ...
            PetType: appSettings.SelectedPetType,
            PetTypeId: appSettings.SelectedPetType.PetTypeId,
            lookup: appSettings.lookup
        };
        //end reset model
        //render template
        petClassificationTemplate.link("#modal-dialog .modal-body", appSettings.model);

        //init form validation
        form = $('#form-petClassification');
        iniValidation();
        //end init form
    ...
    var Edit = function () {
        if ($(this).attr("data-value") != "") {
            ...
            var petClassificationTemplate = $.templates('#petType-template');
            $("#modal-dialog").find('.modal-title').html('Update Pet Classification');
            ...
            api.getById($(this).attr("data-value")).done(function (data) {
                ...
                appSettings.model.PetTypeId = data.Data.PetType.PetTypeId;
            ...
            Swal.fire({
                ...
                .then((result) => {
                    if (result.value) {
                        ...
                        $.ajax({
                            url: app.appSettings.petDatingWebURI + "/PetClassification/",
                            cache: false,
                            type: 'POST',
                            contentType: false,
                            processData: false,
            ...
            Swal.fire({
                ...
                .then((result) => {
                    if (result.value) {
                        ...
                        circleProgress.show(true);
                        $.ajax({
                            url: app.appSettings.petDatingWebURI + "/PetClassification/" + $(this).attr("data-value"),
                            type: "DELETE",
                            contentType: "application/json;charset=utf-8",
                            dataType: "json",
                            success: function (result) {
